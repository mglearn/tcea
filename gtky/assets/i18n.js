/* TCEA "Getting to Know You" gallery — i18n engine.
   ---------------------------------------------------------------------------
   Adapted from the TCEA Freedom Days Breakouts i18n engine (a shared language
   switcher for self-contained educational pages).

   Three languages (English/Español/Tiếng Việt), a shared preference
   (localStorage `tcea.gtky.lang`), ?lang= share links, an auto-built <select>
   picker, and a `breakout-i18n:changed` event so the gallery can re-render its
   worksheet cards in the chosen language on change.
...
(function () {
  ...
  const STORAGE_KEY = 'tcea.gtky.lang';
