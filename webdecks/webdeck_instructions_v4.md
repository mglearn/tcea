# WebDeck Project Instructions v4.0
...
**What changed in v4.0:** This version adds a movable speaker notes window as the preferred notes experience. The control should read `Notes ↗`, and the `W` key should open or focus that movable notes window so the presenter can place it on a second monitor or off to the side. Version 4.0 keeps backward compatibility with v3 decks by preserving the slide-up notes panel and the `N` shortcut as a fallback or alternate notes view. It keeps the v3 single-file runtime, ShowSplat-friendly iframe behavior, responsive and accessibility rules, PPTX/image import guidance, and validation checklist.
...
- **Safe movable notes window:** The `Notes ↗` window may use `window.open()` and `BroadcastChannel`, but the deck must fall back to the in-window slide-up notes panel if pop-ups are blocked or unsupported.
...
<body>
  ...
  <!-- v4 optional: create the movable speaker notes window with window.open(), not an external file. -->
  <script>
...
Version 4.0 must remain compatible with v3-style decks and controls.

Rules:

- Existing `SLIDES` data remains valid. Do not change the `title`, `html`, or `notes` field names.
- Existing v3 slide-up notes panels remain valid. Do not remove the `N` shortcut unless the user explicitly asks for a notes-window-only deck.
- Existing `Notes` buttons may keep working as slide-up panel toggles, but new v4 decks should label the primary notes control `Notes ↗`.
- The `Notes ↗` control should first try to open or focus the movable notes window. If that fails, it should open the slide-up notes panel.
- Existing presenter mode behavior remains valid. The v4 notes window should not break the v3 presenter console.
- The audience view must remain usable if the movable notes window, presenter window, `BroadcastChannel`, or fullscreen request fails.
...
- `W` opens or focuses the movable speaker notes window.
- `N` toggles the in-window slide-up notes panel for backward compatibility.
...
- Use `Notes ↗` as the default label for the notes control in v4 decks. If screen width is tight, `Notes` is acceptable as long as the accessible label says `Open speaker notes in a movable window`.
- If pop-ups are blocked, the `Notes ↗` button must fall back to the in-window slide-up notes panel.

### Speaker Notes

Version 4.0 requires two compatible ways to view notes. The movable notes window is preferred. The slide-up panel remains supported for older v3 decks, pop-up-blocked browsers, and single-screen use.

#### Movable notes window

Include a `Notes ↗` button and a `W` keyboard shortcut that open or focus a separate speaker notes window. The presenter should be able to move that window off to the side during a presentation.

Rules:

- Build the notes window from the same single HTML file. Do not require a separate notes file, server route, storage permission, or asset folder.
- Use `window.open()` for the notes window. A same-origin generated document, same-file query parameter, or `about:blank` document written by the opener is acceptable.
- Keep the notes window synced to the current slide with `BroadcastChannel`, `postMessage`, or a safe same-window reference.
- Show the current slide title, current slide number, total slide count, and current slide notes.
- Update the notes window every time the slide changes.
- If the notes window is already open, `W` and `Notes ↗` should focus it instead of opening duplicates.
- If the notes window has been closed, the next `W` or `Notes ↗` action should open a new one.
- If pop-ups are blocked or `window.open()` returns `null`, fall back to the in-window slide-up notes panel.
- The notes window must not navigate or alter the audience view.
- The notes window should use readable, high-contrast text, large enough for presenter use, and it should scroll if notes are long.

#### Backward-compatible slide-up notes panel

Include a notes panel that slides up over the deck. This is the v3 behavior and remains valid in v4.
...
- The `N` key should toggle this panel.
- The panel is also the required fallback when the movable notes window cannot open.
...
Warn the user in notes or final instructions that the browser may require pop-ups to be allowed for presenter mode and the movable notes window.

Presenter mode and the movable notes window are related but separate features. Presenter mode opens or converts a presenter console. The movable notes window opens only the current slide notes so the presenter can place those notes wherever they are useful.

### Help Overlay

Include a help overlay showing all keyboard shortcuts and controls. It should open and close with `?` or `h`, and close with Escape. The help overlay must list `W` for the movable speaker notes window and `N` for the backward-compatible slide-up notes panel.
...
- Use `aria-expanded` on the in-window notes toggle and a useful accessible label on the `Notes ↗` button, such as `Open speaker notes in a movable window`.
...
- Update dots, counter, progress, the slide-up notes panel, the movable notes window, and presenter view on every slide change.
- Pause click navigation while overlays or the slide-up notes panel are open.
...
- Keep `BroadcastChannel` message types specific enough that presenter mode and the notes window do not misread each other's messages.
...
- Touch swipes work.
- The `Notes ↗` button opens or focuses a movable notes window.
- The `W` key opens or focuses a movable notes window.
- The movable notes window updates when the slide changes and does not create duplicates.
- If the movable notes window is blocked, the deck falls back to the slide-up notes panel.
- The `N` key still toggles the slide-up notes panel for backward compatibility.
...
```text
Build me a WebDeck following WebDeck Project Instructions v4.0.
...
Required features: keyboard navigation, swipe navigation, guarded left/right-third click navigation, clickable dots, slide counter, floating rounded-pill control bar above the bottom edge, top progress bar, `Notes ↗` button and `W` shortcut for a movable speaker notes window, backward-compatible slide-up speaker notes panel with `N` shortcut and pointer-events disabled while closed, help overlay, fullscreen button, presenter mode with BroadcastChannel sync, graceful fallback when pop-ups or fullscreen are blocked, and hash deep linking that also responds to live hashchange.
...
Before finishing, validate: no console errors, all slides render, controls work, `Notes ↗` and `W` open or focus the movable speaker notes window, the `N` slide-up notes fallback still works, notes do not block clicks when closed, hash links work, small screens do not clip content, dark surfaces have readable text, and no required asset is external.
...
5. A warning if presenter mode or the movable notes window requires pop-ups to be allowed.
