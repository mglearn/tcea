function reviewToMarkdown(review, rubric){
  ...
  if(review.ai?.used){ lines.push('> AI-assisted draft. Human reviewer must verify all quoted evidence and final scores.',''); }
  lines.push('## Scores','','| # | Category | Score | Evidence | Gaps |','|---|---|---:|---|---|');
  rubric.categories.forEach(c=>{
    const row = review.categories?.[c.number] || {};
    lines.push(`| ${c.number} | ${c.code} — ${c.name} | ${row.score ?? ''} | ${String(row.evidence||'').replace(/\n/g,'<br>')} | ${String(row.gaps||'').replace(/\n/g,'<br>')} |`);
  });
  lines.push('', '## Documents Reviewed');
  lines.push(`- Privacy Policy pasted: ${review.docs.privacyPolicy ? 'Yes' : 'No'}`);
  lines.push(`- Terms of Service pasted: ${review.docs.terms ? 'Yes' : 'No'}`);
  lines.push(`- Addendum / NDPA pasted: ${review.docs.addendum ? 'Yes' : 'No'}`);
  lines.push(`- Security / compliance document pasted: ${review.docs.security ? 'Yes' : 'No'}`);
  if(review.docs.notes){ lines.push('', '## Reviewer Notes', review.docs.notes); }
...
function reviewToCsv(review, rubric){
  ...
  const rows = [['#','Code','Category','Score','Evidence','Gaps']];
  rubric.categories.forEach(c=>{const row=review.categories?.[c.number]||{}; rows.push([c.number,c.code,c.name,row.score||'',row.evidence||'',row.gaps||'']);});
...
async function exportDocx(review, rubric){
  ...
  const tableRows = [new TableRow({children:['#','Category','Score','Evidence','Gaps'].map(h=>new TableCell({children:[new Paragraph({children:[new TextRun({text:h,bold:true})]})]}))})];
  rubric.categories.forEach(c=>{
    const row=review.categories?.[c.number]||{};
    tableRows.push(new TableRow({children:[String(c.number),`${c.code} — ${c.name}`,String(row.score||''),row.evidence||'',row.gaps||''].map(x=>new TableCell({children:[new Paragraph(String(x))]}))}));
  });
  children.push(new Table({width:{size:100,type:WidthType.PERCENTAGE}, rows:tableRows}));
  children.push(new Paragraph({text:'Reviewer Notes', heading:HeadingLevel.HEADING_1}));
  children.push(new Paragraph(review.docs.notes||''));
