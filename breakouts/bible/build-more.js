...
/* build-more.js — expand the "Bible as Literature" suite.
   Reads bible/grades/_specs-<band>.json (English breakout specs authored per band),
   and for each spec emits:
     1) grades/<band>-<slug>.js         — window.BREAKOUT (English content + shared
                                          translated UI; story content falls back to
                                          English in the 6 other languages)
     2) grades/<band>-<slug>-student.html — the shared-engine student page
   It also writes grades manifest breakouts.js (window.BIBLE_LIB) listing every
   breakout in the suite (the 4 originals + the generated ones) for the landing page.
...
function buildContent(spec) {
  ...
  const locks = spec.locks.map((l, i) => {
    ...
    if (l.type === 'seq') {
      base.pads = l.pads.map((p, j) => ({ k: p.k, e: p.e, c: PAD_COLORS[j % PAD_COLORS.length] }));
      base.answer = l.answer;
    } else if (l.type === 'mc') {
      base.options = l.options; base.answerIndex = l.answerIndex;
    } else if (l.type === 'multi') {
      base.items = l.items.map(it => ({ t: it.t, strong: !!it.strong }));
    } else if (l.type === 'word') {
      const seen = new Set(); const ans = [];
      for (const a of l.answer) { const v = String(a).trim().toLowerCase(); if (v && !seen.has(v)) { seen.add(v); ans.push(v); } }
      base.answer = ans;
    }
    base.reason = l.reason;
    return base;
  });
  return { en: { clues, locks } };
}
...
for (const band of BANDS) {
  ...
  for (const spec of specs) {
    ...
    const obj = { id: `bible-${band.key}-${spec.slug}`, band: band.label, confetti: CONFETTI, UI: buildUI(band, spec), CONTENT: buildContent(spec) };
...
console.log(`build-more: wrote ${written} breakouts (+ student pages), ${lib.length} library entries.`);
