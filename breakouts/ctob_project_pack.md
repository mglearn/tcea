...
- Give every clue an `id` (`c1…c6`) and every lock an `id` (`L1…L4`). A missing `id` renders
  nothing — and only a real-browser solve will catch it (see §14).
...
## 12. Catalog / manifest & site integration
...
## 14. Build & verify workflow (do NOT skip verification)
...
## 16. Copy-paste starter prompt for a chatbot
...
> link a passcode-request form). Respect §8 (contrast, RTL logical properties, descriptive alt
> text, `onerror` image guard). **Verify by solving every breakout in a headless browser in English
> + one non-English + one RTL language, and by the structural checks in §14, before declaring done.**

---

*Provenance: distilled from the TCEA "Fourth of July," "July 5 & Other Black Freedom Holidays,"
CLEAR, and "Bible as Literature" (24-breakout, K–12, 7-language) critical thinking breakout suites.
